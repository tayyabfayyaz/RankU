...
interface FacebookFormProps {
  data: FacebookData
  setData: (data: FacebookData) => void
  onGenerate: () => void
  isLoading: boolean
}

export function FacebookForm({ data, setData, onGenerate, isLoading }: FacebookFormProps) {
  ...
  return (
    <div className="space-y-6">
      <Card className="p-6 border-2 border-dashed border-blue-300">
        <div className="flex flex-col items-center justify-center py-12 cursor-pointer hover:bg-blue-50 rounded-lg transition">
          ...
          <input type="file" accept="image/*" onChange={handleImageChange} className="hidden" id="image-upload-fb" />
          <label htmlFor="image-upload-fb">
      ...
      <Card className="p-6 space-y-4">
        ...
        <div>
          ...
          <Textarea
            ...
            className="border-blue-200 focus:border-blue-600 min-h-32"
          />
        </div>
        ...
        <Button
          onClick={onGenerate}
          disabled={isLoading}
          className="w-full bg-gradient-to-r from-blue-600 to-blue-800 hover:from-blue-700 hover:to-blue-900 text-white font-semibold"
        >
